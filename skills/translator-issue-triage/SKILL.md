...
8. Post exactly one concise comment unless the user explicitly requests
   multiple comments. Invocation of this skill with an issue number or URL is
   permission to post the triage comment and add justified assignees.
...
only the exact data needed to support the conclusion.
